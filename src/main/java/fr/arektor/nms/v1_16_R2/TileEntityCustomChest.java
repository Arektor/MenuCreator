...
public class TileEntityCustomChest extends TileEntityChest implements CustomGui {
	...
	protected TileEntityCustomChest(int length) {
		super();
		this.length = length;
	...
	public TileEntityCustomChest(String title, int length) {
		this(length);
	...
	protected Container createContainer(int i, PlayerInventory playerinventory, HumanEntity owner) {
		CustomGui gui = new ContainerChest_Custom((Player) owner, i, playerinventory, this.length*9, this);
		this.addMirror(gui);
